# Makefile for Computer Vision HW4
...
P4SRC = p4.c
P5SRC = p5.c
MATRIXTESTSRC = matrixtest.c
SEEDSRC = make_seeds.c
...
UTILS = vision_utilities.c util.c
UTILSH = vision_utilities.h util.h
...
# 'make matrixtest' builds matrixtest only
matrixtest: $(MATRIXTESTSRC) $(UTILS)
	$(CC) $(CFLAGS) -o $@ $(MATRIXTESTSRC) $(UTILS) $(LFLAGS)

seeds:  $(SEEDSRC) $(UTILS)
	$(CC) $(CFLAGS) -o $@ $(SEEDSRC) $(UTILS) $(LFLAGS)

# the rule for building the vision utilities
vision_utilities.o: vision_utilities.c utils.c vision_utilities.h utils.h
	$(CC) -c -Wall -pedantic -o $@ vision_utilities.c utils.h

# 'make submit' creates a tar of all files in the directory
# NOTE 1: It also runs 'make clean' before submission.
 submit: clean
	tar -czvf $(UNI)-hw4.tar.gz $(MATRIXTESTSRC) $(P1SRC) $(P2SRC) $(P3SRC) $(P4SRC) $(P5SRC) $(UTILS) $(UTILSH) vase_seed.txt sphere_seed.txt Makefile README
